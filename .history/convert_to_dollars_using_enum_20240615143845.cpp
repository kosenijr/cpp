...
{
    // variables
    constexpr double amount_input = 0.00;
    constexpr char unit = ' ';
    // prompting of user
    ...
    // call functions
    // unit
    constexpr selected_unit = char_to_currency(unit);
    // rate
    constexpr selected_rate = get_conversion_rate(currency);
    // conditionals
    if (selected_unit == null || selected_rate == 0.00) {
        cout << "The unit selected is not known within our system. \n";
    } else {
        cout << "The selected amount of " << amount_input << " " << selected_unit << " is " << fixed << setprecision(2) << amount_input/selected_rate << " dollars. \n"; 
