...
int main()
{
   ...
   double amount_input = 0.00;
    ...
    Currency currency_unit = char_to_currency(unit);
    // rate
    ...
    {
        cout << "The selected amount of " << amount_input << " " << char_to_currency(unit) << " is " << fixed << setprecision(2) << amount_input / conversion_rate << " dollachar_to_currency(unit)
    }
}